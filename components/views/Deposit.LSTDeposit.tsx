...
function LSTDepositForm({token, amount, onUpdateAmount, isDisabled}: {
	token: TLST,
	...
	return (
		<div className={'lg:col-span-4'}>
			<div className={cl('grow-1 flex h-10 w-full items-center justify-center rounded-md p-2', isDisabled ? 'bg-neutral-200' : 'bg-white')}>
				...
				<div className={'ml-2 flex flex-row items-center space-x-2'}>
					...
					<button
						...
						className={cl('px-2 py-1 text-xs rounded-md border border-purple-300 transition-colors bg-purple-300 text-white', isDisabled ? 'opacity-0 pointer-events-none' : '')}>
						{'Max'}
					</button>
				</div>
			</div>
