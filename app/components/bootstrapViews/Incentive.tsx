...
function IncentiveGroupBreakdownItem({item}: {item: TIncentives}): ReactElement {
	return (
		<div
			...
			<div className={'col-span-2 flex items-center justify-end'}>
				<p className={'font-number pr-1 text-xxs md:text-xs'}>{`$${formatAmount(item.value, 2, 2)}`}</p>
			</div>
			<div className={'col-span-2 flex items-center justify-end'}>
				<p className={'font-number pr-1 text-xxs md:text-xs'}>{`${formatPercent(item.estimatedAPR, 2)}`}</p>
			</div>
		</div>
	);
}
					...
					<div
						...
						<div className={'pt-2 md:pt-0'}>
							...
							<small
								suppressHydrationWarning
								className={'pl-2 pt-1 text-xs text-neutral-600'}>
								{`You have ${formatAmount(balanceOf?.normalized || 0, 2, 6)} ${
									tokenToUse?.symbol || '-'
								}`}
							</small>
						</div>
