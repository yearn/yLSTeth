...
import ViewWithdraw from '@yUSD/components/views/Withdraw';
import useBasket from '@yUSD/contexts/useBasket';
import useLST from '@yUSD/contexts/useLST';
import {STYUSD_TOKEN, YUSD_TOKEN} from '@yUSD/tokens';
...
function HeadingUserPosition(): ReactElement {
	...
	const basketTokenBalance = useMemo(
		() => getBalance({address: YUSD_TOKEN.address, chainID: YUSD_TOKEN.chainID}),
		[getBalance]
	);
	...
	const basketTokenStBalance = useMemo(
		() => getBalance({address: STYUSD_TOKEN.address, chainID: STYUSD_TOKEN.chainID}),
		[getBalance]
	);
	...
	return (
		<div className={'divider mt-6 grid grid-cols-1 space-y-2 border-t-2 pt-6'}>
			<div>
				<small className={cl('text-xs', basicLighterColorTransition)}>{'Your yUSD'}</small>
				<b
					suppressHydrationWarning
			...
			<div>
				<small className={cl('text-xs', basicLighterColorTransition)}>{'Your st-yUSD'}</small>
				<span className={'flex w-full items-center justify-between whitespace-nowrap'}>
					{isLoadingBalance || !balances || Object.keys(balances).length === 0 ? (
						...
						<>
							...
							<RenderYBasketValue amount={basketTokenStBalance.raw} />
						</>
					)}
				</span>
			</div>
			...
			{lockedTokens && lockedTokens >= 0n ? (
				<div>
					<small className={cl('text-xs', basicLighterColorTransition)}>{'Your bootstrap st-yUSD'}</small>
					<span className={'flex w-full items-center justify-between whitespace-nowrap'}>
						<b
							suppressHydrationWarning
							className={cl(
						...
						<RenderYBasketValue amount={toBigInt(lockedTokens)} />
					</span>
				</div>
			) : null}
		</div>
	...
	return (
		<div
			...
			<div
				...
				<h1 className={cl('text-5xl md:text-8xl font-black', basicColorTransition)}>{'yUSD'}</h1>
			</div>
								...
								if (tab.slug === 'deposit-eth-leg') {
									return (
										<Link
											...
											href={'https://swap.cow.fi/#/1/swap/ETH/st-yUSD'}
											target={'_blank'}>
											<p
												title={tab.label}
